"""Realtime live-trading dashboard. Reads live_store (sqlite WAL) and serves a
self-contained styled page (dark theme, canvas charts, no external deps):
hero equity/PnL, live equity curve, per-pair cards (target gauge, premium,
prediction, sparkline), trade history, feed health, retrain history."""
...
header{display:flex;align-items:center;gap:12px;padding:14px 20px;border-bottom:1px solid var(--bd);position:sticky;top:0;
background:rgba(10,13,19,.85);backdrop-filter:blur(8px);z-index:5}
h1{font-size:15px;margin:0;font-weight:650;letter-spacing:.3px} h1 .dot{color:var(--blu)}
...
.panel{background:var(--card);border:1px solid var(--bd);border-radius:14px;padding:14px 16px}
.panel h2{font-size:12px;margin:0 0 10px;color:var(--mut);text-transform:uppercase;letter-spacing:.6px;font-weight:600}
...
td,th{padding:5px 8px;text-align:right;border-bottom:1px solid #161c27;white-space:nowrap}
...
<header>
 <h1><span class=dot>◆</span> HL live engine</h1>
...
<main>
 ...
 <div class=row>
  <div class=panel><h2>эквити</h2><canvas id=eq height=170></canvas></div>
  <div class=panel><h2>модель / ретрейны</h2><div id=models></div></div>
 </div>
 <div class=panel style=margin-bottom:16px><h2>пары · сигналы · предсказания</h2><div class=cards id=syms></div></div>
...
function line(cv,xs,ys,color){
 const dpr=devicePixelRatio||1,w=cv.clientWidth,h=cv.height;cv.width=w*dpr;cv.style.height=h+'px';
 const g=cv.getContext('2d');g.scale(dpr,dpr);g.clearRect(0,0,w,h);
 if(ys.length<2){g.fillStyle='#5d677d';g.font='12px sans-serif';g.fillText('ожидание данных…',14,h/2);return;}
 const pad=8,lo=Math.min(...ys),hi=Math.max(...ys),rng=(hi-lo)||1;
 const X=i=>pad+(w-2*pad)*i/(ys.length-1), Y=v=>h-pad-(h-2*pad)*(v-lo)/rng;
 const grd=g.createLinearGradient(0,0,0,h);grd.addColorStop(0,color+'40');grd.addColorStop(1,color+'00');
 g.beginPath();g.moveTo(X(0),Y(ys[0]));ys.forEach((v,i)=>g.lineTo(X(i),Y(v)));
 g.lineTo(X(ys.length-1),h-pad);g.lineTo(X(0),h-pad);g.closePath();g.fillStyle=grd;g.fill();
...
function spark(cv,ys,color){
 const dpr=devicePixelRatio||1,w=cv.clientWidth||200,h=cv.height;cv.width=w*dpr;cv.style.height=h+'px';
 const g=cv.getContext('2d');g.scale(dpr,dpr);g.clearRect(0,0,w,h);
 if(ys.length<2)return;const lo=Math.min(...ys),hi=Math.max(...ys),rng=(hi-lo)||1;
 g.beginPath();ys.forEach((v,i)=>{const x=w*i/(ys.length-1),y=h-2-(h-4)*(v-lo)/rng;i?g.lineTo(x,y):g.moveTo(x,y)});
 g.strokeStyle=color;g.lineWidth=1.4;g.stroke();
 const zy=h-2-(h-4)*(0-lo)/rng;if(0>=lo&&0<=hi){g.strokeStyle='#2c3444';g.setLineDash([2,3]);g.beginPath();g.moveTo(0,zy);g.lineTo(w,zy);g.stroke();g.setLineDash([])}
}
...
async function tick(){
 ...
 if(!s){$('clock').textContent='нет данных (движок не запущен?)';$('hero').innerHTML='<div class=empty>движок ещё не писал состояние — запусти agents-hltrade-engine</div>';return;}
 ...
 $('clock').textContent='обновлено '+((Date.now()-s.ts_ms)/1000).toFixed(0)+'s назад · sec '+s.second;

 const eq=Object.values(s.dex_equity||{}).reduce((a,b)=>a+b,0);
 if(startEq===null&&eq>0)startEq=eq;
 const pnl=startEq?eq-startEq:0, pnlpct=startEq?pnl/startEq*100:0;
 ...
 $('hero').innerHTML=`
  <div class=metric><div class=l>эквити (Σ dex)</div><div class="v num">${money(eq)}</div><div class=s>${dexr}</div></div>
  <div class=metric><div class=l>сессионный PnL</div><div class="v num ${sgn(pnl)}">${pnl>=0?'+':''}${money(pnl)}</div><div class="s ${sgn(pnl)}">${pnlpct>=0?'+':''}${f(pnlpct)}% · от старта ${money(startEq)}</div></div>
 ...
 const eqs=m.map(x=>x.equity).filter(x=>x>0);
 line($('eq'), eqs.map((_,i)=>i), eqs, '#7aa2f7');

 const syms=Object.entries(s.symbols||{});
 $('syms').innerHTML=syms.map(([k,v])=>{
  const prem=(v.stock&&v.perp)?((v.stock/v.perp-1)*100):null;
  const tw=Math.min(Math.abs(v.target),1)*50;
  const fill=v.target>=0?`left:50%;width:${tw}%;background:var(--grn)`:`right:50%;width:${tw}%;background:var(--red)`;
  return `<div class=sym>
   <div class=hd><span class=nm>${k.split(':')[1]}</span><span class=dex>${k.split(':')[0]} · ${v.ready?'ready':'…'}</span></div>
   ...
   <div class=kv><span class=k>нотионал</span><span class=num>${money(v.notional)}</span></div>
   <canvas height=34 data-sym="${k}"></canvas></div>`;}).join('');
 ...
 $('trades').innerHTML='<tr><th>время<th>пара<th>сторона<th>размер<th>цена<th>нотионал<th>статус</tr>'+
  (tr.length?tr.slice(0,60).map(t=>`<tr><td class=num>${new Date(t.ts_ms).toLocaleTimeString()}</td><td>${t.symbol.split(':')[1]}</td>
   <td class=${t.side=='buy'?'pos':(t.side=='sell'?'neg':'flat')}>${t.side}</td><td class=num>${f(t.size,4)}</td>
   <td class=num>${f(t.price,3)}</td><td class=num>${money(t.notional)}</td><td class=flat>${t.result||t.kind}</td></tr>`).join('')
   :'<tr><td colspan=7 class=empty>сделок ещё нет</td></tr>');

 $('models').innerHTML=md.length?'<table><tr><th>время<th>val_corr<th>scale<th>rows<th>live</tr>'+
   md.slice(0,10).map(x=>`<tr><td class=num>${new Date(x.ts_ms).toLocaleTimeString()}</td><td class="num pos">${x.summary.val_corr}</td><td class=num>${x.summary.scale}</td><td class=num>${(x.summary.rows||0).toLocaleString()}</td><td class=num>${(x.summary.live_symbols||[]).length}</td></tr>`).join('')+'</table>'
   :'<div class=empty>ретрейнов ещё нет<br><span style=font-size:11px>baseline-модель из production_ensemble.pt</span></div>';
}
